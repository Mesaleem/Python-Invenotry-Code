...
def fetch_inventory(next_token=None, attempt=1):
    access_token = get_access_token()
    headers = {
    ...
    url = f"{SP_API_BASE_URL}/listings/2021-08-01/items/{SELLER_ID}"
    print(f"Fetching listings with URL: {url}, Params: {params}")
    ...
    if response.status_code == 200:
        ...
        print("Full Response:", json.dumps(response_data))
        
        items = response_data.get("items", [])
        next_token = response_data.get("pagination", {}).get("nextToken")
        print(f"Items Fetched: {len(items)}, Next Token: {next_token}")
        
        return response_data
    elif response.status_code == 429:
        print(f"Rate Limit Exceeded (429). Retrying after {2 ** attempt}s... (Attempt {attempt}/3)")
        time.sleep(2 ** attempt)
        if attempt < 3:
            return fetch_inventory(next_token, attempt + 1)
        else:
            raise Exception("Max retries exceeded for rate limit.")
    elif response.status_code in [401, 403]:
        print(f"Token Error ({response.status_code}): {response.text}")
        raise Exception("Token expired or invalid. Renew REFRESH_TOKEN.")
    else:
        print(f"Listings Error Response ({response.status_code}): {response.text}")
        raise Exception("Failed to fetch listings inventory")
    ...
    try:
        ...
        while True:
            try:
                ...
                print(f"Page {page_count + 1} Full Response:", json.dumps(inventory_data, indent=2))
                items = inventory_data.get("items", [])
                
                # Check used condition items
                used_count = sum(1 for item in items if "used" in item.get("summaries", [{}])[0].get("conditionType", "").lower())
                print(f"Used condition items in page: {used_count}")
                ...
                page_count += 1
                print(f"Page {page_count} SKUs: {sku_list}")
                print(f"Page {page_count} Next Token: {next_token}")
                ...
                for item in items:
                    ...
                    if summaries:
                        ...
                        if "used" in condition.lower():
                            lowest_price = get_lowest_price(asin)
                ...
                if not next_token:
                    ...
                    if next_token in seen_tokens:
                        print(f"⚠️ Repeated nextToken detected: {next_token}. Breaking loop.")
                        print("Last Page SKUs:", sku_list)
                        break
                ...
                if "Rate Limit Exceeded" in str(e):
                    print(f"Rate Limit Error in get_all_inventory. Retrying after {2 ** attempt}s... (Attempt {attempt}/3)")
                    time.sleep(2 ** attempt)
                    if attempt < 3:
                        continue
                    else:
                        raise Exception("Max retries exceeded for rate limit in get_all_inventory.")
                else:
                    print(f"Error in loop: {str(e)}")
                    raise e

        print(f"Inventory fetching complete. Total items: {item_count}")
